...
\addcontentsline{toc}{section}{Part B: Designing the Software}
\noindent\rule{\textwidth}{0.4pt}
...
\fancyhead[OL]{\em Part B: Designing the Software}
...
\begin{figure}[h!] 
     \centering 
    %\includegraphics[width=0.5\linewidth]{artificial_noise.png}
...
\addcontentsline{toc}{subsection}{3. Simulated Image Datasets}
\fancyhead[OR]{ Simulated Image Datasets}
\fancyhead[EL]{ Simulated Image Datasets}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{preprocessing_max.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{preprocessing_mean.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{preprocessing_satur.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{preprocessing_sigmoid.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{preprocessing_opening.png}
...
\begin{algorithm}[h!]
\caption{\rule{0pt}{3mm}{} {\bf Quadratic Fit Search} - Our Implementation's Python Pseudo-code}\label{alg:qfs}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{fibo.png}
...
\begin{algorithm}[h!]
\caption{\rule{0pt}{3mm}{} {\bf Fibonacci Ratio Search} - Our Implementation's Python Pseudo-code}\label{alg:fibo}
\Given{  $C(\cdot)$, $\Delta x$, $max\_its$, $C\_tol$,  $initial\_guess$, $x_{min}$, $x_{max}$ }{
$x_{j}s,\ C_{j}s \gets initialize\_points(initial\_guess, x_{min},x_{max}, C)$\; \DontPrintSemicolon{\Comment*[r]{\footnotesize $x_js,\ C_js$ are valid initial point- and respective cost-arrays with 4 points}}
...
\addcontentsline{toc}{subsection}{5. Geometric Algorithms}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{kalk.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{grav.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{histo.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{blaz.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{blaz2.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{ransac.png}
...
We can still further generalize the algorithm by allowing the 2D search for $\vec{g}_{geom}$ to not only use Blazquez's algorithm cost, but a weighted mix of it with the least-square cosine squared fit error (of B.8.8.). We could parametrize this cost with a linear parameter $s\in[0,1]$, where $s=0$ would yield the Blazquez algorithm (B.6.7.) with the final RANSAC modification, while $s=1$ would yield the cosine squared fit algorithm (B.6.8.) with the RANSAC final distillation. The procedure is summarized in Algorithm \ref{alg:Mix} .
...
\newpage
\section*{B.7. Basic Black-Box Algorithms}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{triplet.png}
    \caption{ (a) }
    \label{fig:triplet}
\end{figure}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{embeddings1.png}
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{embeddings2.png}
    \caption{ Samples of the noisy dataset embedded using the trained embedders to 10D, and reduced dimensionally using PCA to 3D. the sub-caption indicates the employed embedding algorithm to 10D.  }
...
\addcontentsline{toc}{subsection}{8. Simulation Fitting Algorithms }
...
\begin{figure}[h!] 
     \centering 
%    \includegraphics[width=0.5\linewidth]{simul.png}
    \caption{ Experimental noisy images and corresponding most similar found theoretical images using the Simulation Fit algorithm.  }
    \label{fig:simul}
\end{figure}
...
An alternative version was implemented following Algorithm \ref{alg:simul2}. I uses the fact that the Blazquez algorithm (B.6.7.) is very good at finding the geometric center of noisy images, with a precision presumably better than the noisy gravicenter we can naively obtain, as we discussed in (B.6.). Then, it is the experimental images that are centered in the geometric centers, before comparing with the simulations, which are centered likewise by default.
...
\addcontentsline{toc}{subsection}{9. Pipeline to Compare The Algorithms and Preprocessings }
...
\addcontentsline{toc}{subsection}{ 10. Performance on Experimental Data }
...
\addcontentsline{toc}{section}{Part C: Designing the Hardware}
...
\fancyhead[EL]{\em Setup to Observe Conical Refraction Rings}
\fancyhead[ER]{\em Bachelor's Thesis: Conical Refraction Polarimeter}
...
\fancyhead[EL]{\em Experimental Ring Non-Idealitites}
...
\section*{C.4. The Polarimeter, a Proof of Concept} 
 \addcontentsline{toc}{subsection}{3. The Polarimeter, a Proof of Concept}
 \fancyhead[OR]{ The Polarimeter, a Proof of Concept}
\fancyhead[EL]{\em The Polarimeter, a Proof of Concept}
...
\subsection*{C.4.1. Cost Analysis and Commercial Polarimeters}
